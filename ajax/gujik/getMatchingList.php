...
*/
    ...
    $sql .= " FROM work_resume_data A, work_join_list B, work_employ_data C, company D ";
    $sql .= " WHERE 1=1 ";
    $sql .= " AND A.uid   = '$uid' ";
    $sql .= " AND A.no    = B.work_resume_data_no ";
    $sql .= " AND B.choice  = 'no' ";
    $sql .= " AND B.work_employ_data_no = C.no ";
    $sql .= " AND C.member_no = D.member_no ";
    $sql .= " GROUP BY C.no ";
    ...
    if( $allPost == "" || $allPost == null ) $allPost = 0;
    ...
    $sql  = " SELECT B.work_resume_data_no, B.work_employ_data_no, C.no as resumeNo, C.title, C.keyword,C.business, C.pay, C.career, C.content, C.wdate, C.time, C.member_no,D.img_url";
    $sql .= " FROM work_resume_data A, work_join_list B, work_employ_data C, company D ";
    $sql .= " WHERE 1=1 ";
    $sql .= " AND A.uid   = '$uid' ";
    $sql .= " AND A.no    = B.work_resume_data_no ";
...
*/
    ...
    if($page != 1) {
      $paging .= '<li><a href="javascript:getMatchingList(1,'.$onePage.')"> << 첫페이지 </a></li>';

    }
    //첫 섹션이 아니라면 이전 버튼을 생성
    if($currentSection != 1) {
      $paging .= '<li><a href="javascript:getMatchingList('.$prevPage.','.$onePage.')">이전</a></li>';
    }

    for($i = $firstPage; $i <= $lastPage; $i++) {
      if($i == $page) {
        $paging .= '<li class="active"><a href="javascript:getMatchingList('. $i .','.$onePage.')">'.$i.'</a></li>'; 
      } else {
        $paging .= '<li><a href="javascript:getMatchingList('. $i.','.$onePage.')">'.$i.'</a></li>'; 
      }
    }

    //마지막 섹션이 아니라면 다음 버튼을 생성
    if($currentSection != $allSection) {
      $paging .= '<li><a href="javascript:getMatchingList('.$nextPage.','.$onePage.')">다음</a></li>';
    }

    //마지막 페이지가 아니라면 끝 버튼을 생성
    if($page != $allPage && $allPage != 0) {
      $paging .= '<li><a href="javascript:getMatchingList('.$allPage.','.$onePage.')">끝페이지 >> </a></li>';
    }
