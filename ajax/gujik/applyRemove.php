<?php
   ...
   if ($ruid == $euid){
    ...
    for ($i=0; $i<count($work_date); $i++) {
       ...
       $sql = "SELECT no FROM work_join_list WHERE ruid = '$ruid' AND work_resume_data_no IN (SELECT no FROM work_resume_data A WHERE A.uid = '$ruid' AND A.delegate = 1) AND work_date = '$work_date[$i]' AND work_employ_data_no = '$work_employ_data_no' AND choice ='no'";
        $result = mysql_query($sql, $ilbang_con);
    ...
    echo json_encode(array('result' => "1" ,'msg'=>'구직 취소 되었습니다. ','work_date'=>$work_date, 'sql'=>$sql));
?>