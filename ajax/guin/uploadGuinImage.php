<?php
    ...
    if (isset($_FILES['upload']) && !$_FILES['upload']['error']) {
        ...
        if (in_array($_FILES['upload']['type'], $imageKind)) {
            ...
            if (move_uploaded_file ($_FILES['upload']['tmp_name'], "../../guinImage/{$fileName}")) {

                 $imSize=getimagesize('../../guinImage/'.$fileName);
                  
                  if($imSize[0]==144 && $imSize[1]==38){
                ...
                // 5.업로드된 이미지 파일을 DB저장 및 기존이미지 삭제
                    ...
                    $img_url=$row["img_url"]; 
                    //기존이미지 제거
                    unlink('../../guinImage/'.$img_url);
 
                    $sql="UPDATE company SET img_url='$fileName' where uid='$uid'";
                    $result= mysql_query($sql);
                    if($result){
                        $data["image"]=$fileName;
                    ...
                    //파일제거 
                    unlink('../../guinImage/'.$fileName);

                    $data["message"]="파일이미지는 144x38px만 가능합니다.";
