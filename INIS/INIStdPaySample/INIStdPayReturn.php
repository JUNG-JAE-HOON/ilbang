        ...
        try {
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                try {
                    ...
                    $sql = "
                        INSERT INTO inis_result_data (tid, pay_method, result_code, result_msg, appl_date, apple_time, mid, `oid`, good_name, total_price, buyer_name, buyer_tel, buyer_email) 
                        VALUES (
                                  '".@(in_array($resultMap['tid'] , $resultMap) ? $resultMap['tid'] : 'null' )."'
                                , '".@(in_array($resultMap['payMethod'] , $resultMap) ? $resultMap['payMethod'] : 'null' )."'
                                ...
                                , '".@(in_array($resultMap['applDate'] , $resultMap) ? $resultMap['applDate'] : 'null' )."'
                                , '".@(in_array($resultMap['applTime'] , $resultMap) ? $resultMap['applTime'] : 'null' ) ."'
                                , '".$mid."'
                                , '".@(in_array($resultMap['MOID'] , $resultMap) ? substr($resultMap['MOID'],strrpos($resultMap['MOID'],'_')+1) : 'null' )."'
                                , '".@(in_array($resultMap['goodName'] , $resultMap) ? $resultMap['goodName'] : 'null' )."'
                                , '".@(in_array($resultMap['TotPrice'] , $resultMap) ? $resultMap['TotPrice'] : 'null' )."'
