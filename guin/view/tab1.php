<?php
    ...
    while($row = mysql_fetch_array($result)){
        ...
        else if ($row["career"] == "1")   $career = "1년 미만";  
        else if ($row["career"] == "3")   $career = "3년 미만";  
        else if ($row["career"] == "5")   $career = "5년 미만";  
        else if ($row["career"] == "6")   $career = "5년 이상";  
    ...
    if($arr[0] == 0) {
        mysql_query("INSERT INTO recent_employ(employ_no, wdate, work_2nd, title, uid ) VALUES ($employNo, '$wdate', '$work_2nd_nm', '$title', '$uid')");
    } else {
        mysql_query("UPDATE recent_employ SET wdate = '$wdate' WHERE employ_no = $employNo AND uid = '$uid'");
    }

    $sql = "SELECT COUNT(*) AS recentCnt, MIN(no) AS recentNo FROM recent_employ WHERE uid = '$uid'";
    $result = mysql_query($sql);
    $arr = mysql_fetch_array($result);

    $recentCnt = $arr["recentCnt"];

    if($recentCnt > 10) {
        $recentNo = $arr["recentNo"];

        mysql_query("DELETE FROM recent_employ WHERE uid = '$uid' AND no = $recentNo");
    }
        ...
        <div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane active mt10 container pl30" id="gujikTab1">
                <div class="gujikDetSect1 mt50">
                    <div class="border-navy">
                        ...
                        <ul class="detailInfoUl_guin pr">
                            <li class="bb padding">
                                <span class="di w10">간단 소개</span>
                                <span class=""><?php echo  $business ?></span>
                            </li>
                            <li class="bb padding">
                                <span class="di w10">담당자</span>
                                <span class=""><?php echo  $manager ?></span>
                            </li>
                            <li class="padding">
                                <span class="di w10">회사 위치</span>
                                <span class=""><?php echo  $address ?></span>
                            </li>
                            <li class="pa tc" style="left:20px; top:35px;">
                                <?php
                                    $sql = "SELECT kind FROM member WHERE uid = '$euid'";
                                    $result = mysql_query($sql);
                                    $arr = mysql_fetch_array($result);
                                    ...
                                    if($chkKind == "general") {
                                        ...
                                        $sql = "SELECT img_url FROM member_extend WHERE uid = '$euid'";
                                    } else if($chkKind == "company") {
                                        $imgUrl = "../../guinImage/";

                                        $sql = "SELECT img_url FROM company WHERE uid = '$euid'";
                                    }
                                    ...
                                    if(empty($arr[0])) {
                                        $imgUrl = "../../images/144x38.png";
                                    }
                                ?>
                                <img src="<?php echo $imgUrl; ?>" id="" alt="프로필" style="width:205px; height:54px;">
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="gujikDetSect3 oh mb30">
                    <div class="oh">
                        <div class="oh cont">
                            <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 pl15 tl">담당 업무</p>
                            <p class="fl sect2Table di lh40 f12 tc noMargin pdl20"><?php echo  $business ?></p>
                        </div>
                        <div class="oh cont">
                            <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 pl15 tl">담당자 연락처</p>
                            <p class="fl sect2Table di lh40 f12 tc noMargin pdl20"><?php echo  $phone ?></p>
                        </div>
                        <div class="oh cont bg_grey">
                            <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 pl15 tl">근무 일자</p>
                        ...
                        <div class="oh cont">
                            <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 pl15 tl">근무 시간</p>
                            <p class="fl sect2Table di lh40 f12 tc noMargin pdl20"><?php echo  $time ?></p>
                        </div>
                        <div class="oh cont">
                            <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 pl15 tl">금액 (일당)</p>
                            <p class="fl sect2Table di lh40 f12 tc noMargin pdl20">
                                <b class="fc"><?php echo  number_format($pay) ?>원</b>
                            </p>
                        </div>
                        <div class="oh bb">
                            <p class="fl w100 noMargin sect2head di bg_grey lh40 f14 pl15 tl">상세 모집 내용</p>
                        </div>
                        <div class="sect3Inner">
                            <p class="tl p15"><?php echo  $content ?></p>
                        </div>
                    </div>
                </div>
                <div class="tc">
                ...
                <h4 class="mt30 f16 mb10">위치 정보</h4>
                <div class="gujikDetSect4">
                    <div class="oh cont bb">
                        <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 tc">위치 정보</p>
                    ...
                    <div class="oh cont" style="width: 100%">
                        <p class="fl w20 noMargin sect2head di bg_grey lh40 f14 tc mapSecTlt" style="width: 20%">지도 보기</p>
                        <p class="fl sect2Table di lh40 f12 tc noMargin pdl20">
                            <div id="map" style="width:80%;height:495px;"></div>
                                <script>
